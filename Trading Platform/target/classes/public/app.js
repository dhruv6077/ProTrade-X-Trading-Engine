document.addEventListener("DOMContentLoaded", () => {
  ...
  function checkStatus() {
    fetch("/api/status")
      ...
      .then((data) => {
        ...
        // Update latency display with real data
        if (data.latencyMs !== undefined) {
          const latency = parseFloat(data.latencyMs);
          if (latency > 0) {
            latencyValue.textContent = `${latency.toFixed(2)}ms`;
          } else {
            latencyValue.textContent = "~Waiting...";
          }
        }
      })
      .catch(() => {
  ...
  function renderMarketCards(data) {
    ...
    data.forEach((item) => {
      ...
      if (currentPrice > 0) {
        ...
        if (priceHistory[item.symbol].length > 20)
      ...
      if (percentChangeEl) {
        ...
        percentChangeEl.style.color =
          percentChange >= 0 ? "#00e676" : "#ff5252";
      }
  ...
  function renderLogs(filterText) {
    ...
    logContainer.innerHTML = logsToShow
      .map((log) => {
        // Try to match the new format: [Time] [ID:...] [Hash:...] Message
        const extendedMatch = log.match(/^(\[.*?\])\s+(\[ID:.*?\])\s+(\[Hash:.*?\])\s+(.*)/);
        
        let time, message, idSection = "", hashSection = "";
        
        if (extendedMatch) {
            time = extendedMatch[1].replace(/[\[\]]/g, "");
            const id = extendedMatch[2].replace(/[\[\]]/g, "");
            const hash = extendedMatch[3].replace(/[\[\]]/g, "");
            message = extendedMatch[4];
            
            idSection = `<span class="log-id">${id}</span>`;
            hashSection = `<span class="log-hash">${hash}</span>`;
        } else {
        ...
        if (message.includes("BUY")) {
          ...
          className = "system";
            ...
            type = "TRADE";
            className = "system"; // Trade is a system event but distinct
        }
