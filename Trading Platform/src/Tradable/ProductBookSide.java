...
public class ProductBookSide {
    ...
    private final NavigableMap<Price, ArrayList<Tradable>> bookEntries;
     ...
     */
    TradableDTO add(Tradable o) throws InvalidUserInput {
        if (o == null) {
     ...
     */
    TradableDTO cancel(String tradableId) throws InvalidUserInput {
        if (tradableId == null) {
     ...
     */
    TradableDTO removeQuotesForUser(String userName) throws InvalidUserInput {
        if (userName == null) {
    ...
    public Price topOfBookPrice() {
        ...
        try {
            ...
            return (side == BUY) ? bookEntries.lastKey() : bookEntries.firstKey();
        } finally {
     ...
     */
    void tradeOut(Price price, int vol) throws InvalidUserInput, DataValidationException {
        if (price == null) {
    ...
    public String toString() {
        lock.readLock().lock();
        try {
            List<Map.Entry<Price, ArrayList<Tradable>>> bookEntriesList = (side == BUY)
                    ? new ArrayList<>(bookEntries.descendingMap().entrySet())
                    : new ArrayList<>(bookEntries.entrySet());
